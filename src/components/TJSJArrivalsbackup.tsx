...
const TJSJArrivals: React.FC = () => {
  ...
  const handleLimitChange = (gate: string, value: number) => {
    setLimits(prev => ({ ...prev, [gate]: value }));
    fetch('http://localhost:3001/api/limits', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...limits, [gate]: value }),
    });
  };
  ...
  const fetchData = async () => {
    ...
    try {
      const limitsRes = await fetch('http://localhost:3001/api/limits');
      const savedLimits = await limitsRes.json();
